...
function fixKeyCode(key: number) {
	if (firefox) {
		if (key === 163) return Key.FORWARD_SLASH;
		if (key === 173) return Key.DASH;
		if (key === 61) return Key.EQUALS;
		if (key === 59) return Key.SEMICOLON;
	}
...
export class InputManager {
    ...
    private keyHandlers = new Map<Key, () => void>();
    constructor () {
    ...
    get shift() {
        return this.pressedKeys.has(Key.SHIFT);
    }
    get ctrl() {
        return this.pressedKeys.has(Key.CTRL);
    }
    get alt() {
        return this.pressedKeys.has(Key.ALT);
    }
    private keydown = ({ keyCode }: KeyboardEvent) => {
        const code = fixKeyCode(keyCode);
    ...
    onPressed(key: Key, handler: () => void) {
    ...
    onReleased(key: Key, handler: () => void) {
        this.keyHandlers.set(key, () => {
            if (!this.pressedKeys.has(key)) {
                handler();
            }
